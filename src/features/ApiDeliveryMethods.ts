...
const deliveryMethodsApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getPostalPunct: builder.query<
      {
        [key: string]: [
          {
            PostOffice: IPostalPunct;
            PostOfficeAddress: IPostalPunktAddress;
          }
        ];
      },
      number | void
    ...
    getPostalPunctWorkTime: builder.query<
      { [key: string]: IPostalPunktWorkTime[] },
      number | void
    >({
