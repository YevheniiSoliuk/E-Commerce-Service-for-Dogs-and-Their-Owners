      ...
      state.positions.forEach((position) => {
        if (position.product.id === payload.id) {
          ...
          if (payload.discountAmount !== null) {
            state.totalDiscount +=
              payload.price * (payload.discountAmount / 100);
          }
        }
      });
          ...
          if (position.product.discountAmount !== null) {
            state.totalDiscount -=
              ...
              (position.product.price *
                (position.product.discountAmount / 100));
          }
        }
      });

      state.positions = state.positions.filter(
        (position) => position.product.id !== payload
      );
      state.positionsAmount -= 1;
    },
    incrementProductAmountByValue: (
      state,
      { payload }: PayloadAction<{ product: IProduct; value: number }>
    ) => {
        ...
        if (position.product.id === id) {
          ...
          if (payload.product.discountAmount !== null) {
            state.totalDiscount +=
              ...
              (payload.product.discountAmount / 100) *
              value;
          }
        }
      });
    ...
    incrementProductAmount: (
      ...
      state.positions.forEach((position) => {
        if (position.product.id === payload) {
          ...
          if (position.product.discountAmount !== null) {
            state.totalDiscount +=
              position.product.price * (position.product.discountAmount / 100);
          }
        }
      });
    },
