...
export const Input: React.FC<InputProps> = ({id, type, name, value, required, placeholder, width, state, imgSrc, price, action, onChange}) => {
        ...
        <div className="flex relative items-center mx-[30px] my-[10px] mb-[5px]">
          ...
          {name === "payments" ?
            <>
              ...
              <label 
                htmlFor={id} 
                className="ml-[20px] text-[24px] text-green">
                  <img 
                    src={imgSrc} 
                    alt={placeholder} 
            ...
            <>
              ...
              <label 
                htmlFor={id} 
                className="ml-[5px] text-[14px] text-green"
              >
                {placeholder}
              </label>
            </>
        ...
        <div className="flex relative items-center mx-5 mb-[30px]">
          {state === true ? 
            <>
              ...
              />
              <span 
                className="inline-block relative h-[30px] w-[50px] bg-yellow/80 rounded-full border-2 border-green ml-[-20px] before:content[''] before:absolute before:bg-orange before:top-[-2px] before:left-[-2px] before:w-[30px] before:h-[30px] before:border-2 before:border-green before:rounded-full before:transition-all before:ease-linear before:duration-200 peer-checked:before:translate-x-[17px] peer-checked:bg-orange">
              </span>
            </> :
            <>
              <input 
              ...
              />
              <span 
                className="inline-block relative h-[30px] w-[50px] bg-yellow/80 rounded-full border-2 border-green ml-[-20px] before:content[''] before:absolute before:bg-orange before:top-[-2px] before:left-[-2px] before:w-[30px] before:h-[30px] before:border-2 before:border-green before:rounded-full before:transition-all before:ease-linear before:duration-200">
              </span>
            </>
          }
          <label 
        ...
        <div className="flex flex-wrap justify-start flex-col text-left">
          ...
          {required ? 
            <input 
              ...
              className={width + " h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0 placeholder:text-green/50 invalid:border-dark_red"} required
            /> :
            <input 
              id={id} 
              type={type} 
              ...
              onChange={onChange} 
              placeholder={placeholder} 
              className={width + " h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0 placeholder:text-green/50 invalid:border-dark_red"}
            />
          }
        </div>
      }
