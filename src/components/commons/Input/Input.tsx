...
const Input = ({id, type, name, value, required, placeholder, width, state, imgSrc, price, action, onChange}: InputProps) => {
    ...
    <>
      ...
      {type !== "checkbox" && type !== "radio" && type !== "file" && type !== "toggle" &&
        <div className="flex flex-wrap justify-start flex-col text-left">
          ...
          {required ? <input id={id} type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className={width + " h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0 placeholder:text-green/50 invalid:border-dark_red"} required/> :
          <input id={id} type={type} name={name} value={value} onChange={onChange} placeholder={placeholder} className={width + " h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0 placeholder:text-green/50 invalid:border-dark_red"}/>}
        </div>
      }
    </>
  )
