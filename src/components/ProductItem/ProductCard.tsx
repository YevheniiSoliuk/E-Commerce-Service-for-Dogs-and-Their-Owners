...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const { id, title, brand, images, price, basePrice, discountAmount, rate } = {
    ...product
  };
    ...
    <section className="flex flex-col justify-start w-[230px] h-[100%] bg-yellow border-2 border-green shadow-lg px-[10px] py-[10px] text-left rounded-[25px] mb-[20px] tracking-[.1em] relative">
      <Link to={`/product/${id}`}>
        {images === undefined ? (
          ...
          <img src={images[0]} alt="Product1" className="w-[100%] mb-[20px]" />
        )}
      </Link>
      {isFavourite(id) ? (
        <svg
      ...
      {discountAmount !== null ? (
        <>
          ...
          <span className="absolute text-white text-[20px] font-bold -rotate-45 left-[2%] top-[3%]">
            {discountAmount}%
          </span>
        </>
      ) : null}
