...
export const ProductListViewItem: React.FC<ProductCardProps> = ({product, action}) => {
  const { id, title, brand, longDescription, images, price, basePrice, discountAmount, rate } = {...product};
  ...
  const addToFavourites = async (id: number) => {
    ...
    .then((payload) => dispatch(setNewFavoriteProducts(
      payload[`Updated product with id: ${id.toString()}`][`Record with id: ${id.toString()} added successfully to favourites!`]
    )))
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col justify-between items-justify w-[60%]">
        ...
        <p className="flex items-center grow text-[14px] text-center my-[20px] ">{longDescription}</p>
        <div className="flex justify-between items-center mb-[10px]">
          {discountAmount !== null ?
            <>
              <p className="text-[24px] mb-[7px] text-dark_red">{price - (price * (discountAmount/100))} zł</p>
