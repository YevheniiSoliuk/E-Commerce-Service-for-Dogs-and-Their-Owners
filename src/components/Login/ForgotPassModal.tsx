...
const ForgotPassModal = ({isOpen, close, goToOtherModal, comebackToPrevModal}: ModalProps) => {
  ...
  return (
    <Popup open={isOpen} closeOnDocumentClick onClose={close}>
      <div className="flex flex-col items-center justify-center w-[600px] h-[100%] bg-dark_green rounded-[20px] border-2 border-green text-base font-lemon text-center text-green">
        ...
        <h2 className="w-[440px] text-green text-[40px] font-semibold mt-[10px] leading-[45px]">Przypomnienie hasła</h2>
        <p className="w-[420px] my-[30px] text-[14px]">Na Twój adres zostanie wysłany e-mail, który zawiera token do zmiany hasła. Zmiana hasła będzie możliwa po wypełnieniu wszystkich danych w formularzu.</p>
        <form method="post" onSubmit={onHandleSubmit}>
          ...
          />
          <Button text="Wyślij email" type="submit" value="send_mail" styles="h-[50px] bg-orange border-2 border-green hover:border-yellow rounded-3xl text-gree text-base font-lemon px-6 py-2 w-[222px]" onClick={goToOtherModal}/>
        </form>
        <span className="block my-[18px] text-[14px] underline hover:text-yellow cursor-pointer" onClick={comebackToPrevModal}>Wróć do logowania</span>
      </div>
    </Popup>
  )
}
