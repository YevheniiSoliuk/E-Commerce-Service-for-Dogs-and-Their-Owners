...
export const CartPopupItem: React.FC<IOrderPosition> = ({
  product,
  amount
}) => {
  const { id, images, title, price, basePrice, discountAmount } = {
    ...product
  };
  ...
  return (
    <section className="flex justify-start items-end relative mb-[30px]">
      ...
      {images ? (
        <img src={images?.[0]} alt={''} className="w-[30%]" />
      ) : (
        <img
      ...
      <div className="w-[40%] ml-[20px]">
        ...
        {discountAmount !== null ? (
          <p className="text-dark_red text-[16px] mb-[5px]">
