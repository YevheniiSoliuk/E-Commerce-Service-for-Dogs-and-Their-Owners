  ...
  return (
    <ul className={className}>
      ...
      {paginationRange.map((pageNumber: number|string) => {
        ...
        return (
          <li key={pageNumber} className={pageNumber === currentPage ? "text-[20px] hover: text-yellow cursor-pointer" : "text-[20px] cursor-pointer"} onClick={() => onPageChange(pageNumber)}>
            {pageNumber}
          </li>
        );
      })}
