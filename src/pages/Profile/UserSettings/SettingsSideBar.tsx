...
export const SettingsSideBar = () => {
  ...
  const id: string | undefined = useSelector(
  ...
  return (
    <aside className="w-[400px] h-[100%] bg-dark_green border-2 border-green rounded-[25px] p-[50px]">
      <h2 className="text-[32px] text-center mb-[50px]">USTAWIENIA</h2>
      <section>
        {settingsLinks.map((link) => (
          <div className="flex items-center mb-[30px]">
            <span
              className="text-[20px] hover:text-yellow hover:cursor-pointer peer"
              onClick={() => {
                dispatch(changeSettingsSection(link.state));
              }}
            >
              {link.title}
            </span>
            <svg
              ...
              viewBox="0 0 24 24"
              fill="currentColor"
              className="hidden w-[20px] h-[20px] fill-green mx-[10px] peer-hover:inline-block peer-hover:fill-yellow"
            >
              <path
                fillRule="evenodd"
      ...
      <section className="flex justify-center w-[100%] mt-[60px]">
        <Button
          text="DEAKTYWUJ KONTO"
          value="disactivate-account"
          styles="h-[50px] bg-dark_red border-2 border-green hover:border-white rounded-3xl 
          text-white text-base font-lemon px-[6px] py-[2px] w-[250px] text-[16px]"
          onClick={() => {
