...
export const SummarySection = () => {
  const basket: ProductCart = useSelector(
    (state: RootState) => state.productCart
  );
  const delivery_price: number | undefined = useSelector(
    (state: RootState) => state.order.deliveryMethod?.deliveryPayment
  );
  ...
  const [addressOfCurrentUser, setAddressOfCurrentUser] =
    ...
    >
          ...
          <>
            <div className="w-[90%] flex items-center justify-between pl-[10%]">
              <img
                ...
                className="w-[15%]"
              />
              <p className="w-[40%] text-left text-[20px] font-normal">
              ...
              <div>
                <p className="text-[20px] text-dark_red text-center font-semibold">
                  {position.product.price} zł
                </p>
                <p className="text-[16px] text-center font-normal">
                  ({position.product.basePrice} zł/kg)
                </p>
              </div>
            </div>
            <div className="w-[90%] h-[2px] bg-green ml-auto mr-auto my-[20px]"></div>
      ...
      <section className="my-[20px] flex flex-col items-center justify-center">
        <p className="w-[70%] flex justify-between items-center">
          <span className="text-[24px] text-left font-semibold">
            Wartość zamówienia:
          </span>
          <span className="text-[24px] text-left text-dark_red font-semibold">
            {basket.total} zł
          </span>
        </p>
        <p className="w-[70%] flex justify-between items-center mt-[10px]">
          ...
          <span className="text-[24px] text-left text-dark_red font-semibold">
            {delivery_price ? delivery_price : 0} zł
          </span>
        </p>
      </section>
      <div className="w-[90%] h-[2px] bg-green ml-auto mr-auto my-[20px]"></div>
      <p className="w-[90%] flex justify-between items-center ml-[20px]">
        ...
        <span className="text-[32px] text-left text-dark_red font-semibold">
          {delivery_price ? delivery_price + basket.total : basket.total} zł
        </span>
      </p>
