...
import { toggleContactForm, checkEmailNewsletter, checkSMSNewsletter, checkTerms } from "../../features/registration/ContactFormSlice";


const Signup = () => {

  const {isCorporation} = useSelector((state: any) => state.contactForm);
  ...
  const [county, setCounty] = useState("Powiat");
  const [community, setCommunity] = useState("Gmina");
        ...
        <form method="post" onSubmit={onHandleSubmit} className="flex justify-between bg-dark_green/80 rounded-2xl border-2 border-green px-[40px]">
          <div>
            ...
            <div className="flex justify-between mb-[20px] ml-[5px]">
              <Input id="firm" type="radio" name="typ" placeholder="Firma" width="w-[340px]" action={()=>{dispatch(toggleContactForm())}}/>
              <Input id="customer" type="radio" name="typ" placeholder="Osoba prywatna" width="w-[340px]" action={()=>{dispatch(toggleContactForm())}}/>
            </div>
            {isCorporation ? 
              <>
                <Input id="firm-name" type="text" name="name" value={firmName} onChange={(e)=>{setFirmName(e.target.value)}} placeholder="Firma" width="w-[340px]"/>
                <Input id="nip" type="text" name="nip" value={nip} onChange={(e)=>{setNip(e.target.value)}} placeholder="NIP" width="w-[340px]"/>
              </> :
              <>
          ...
          <div>
            ...
            <Select id="wojew" name="wojews" values={["Dolnośląskie", "Mazowieckie", "Pomorskie"]} placeholder="Województwo" styles="w-[340px] h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0" icon={false} value={province} onChange={(e)=>{setProvince(e.target.value)}}/>
            <Select id="powiat" name="powiats" values={["Wrocław", "Lublin", "Warszawa"]} placeholder="Powiat" styles="w-[340px] h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0" icon={false} value={county} onChange={(e)=>{setCounty(e.target.value)}}/>
            <Select id="gmina" name="gminas" values={["Wrocław", "Lublin", "Warszawa"]} placeholder="Gmina" styles="w-[340px] h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0" icon={false} value={community} onChange={(e)=>{setCommunity(e.target.value)}}/>
            <Select id="place" name="places" values={["Wrocław", "Lublin", "Warszawa"]} placeholder="Miejscowość" styles="w-[340px] h-[50px] bg-yellow text-green border-2 border-green rounded-[20px] shadow-md mb-[30px] px-[20px] py-[10px] outline-0" icon={false} value={place} onChange={(e)=>{setPlace(e.target.value)}}/>
          ...
          <div>
            ...
            <Input id="check" type="checkbox" name="enews" placeholder="Chcę otrzymywać E-mail Newsletter (możliwość późniejszej rezygnacji)" width="" action={()=>{dispatch(checkEmailNewsletter())}}/>  <br/>
            <Input id="check1" type="checkbox" name="enews" placeholder="Chcę otrzymywać SMS Newsletter (możliwość późniejszej rezygnacji)" width="" action={()=>{dispatch(checkSMSNewsletter())}}/>  <br/>
            <Input id="check2" type="checkbox" name="enews" placeholder="Akceptuję warunki regulaminu i polityki prywatności. Zgadzam się na otrzymywanie informacji dotyczących zamówień w myśl ustawy z dnia 18 lipca 2002r. o świadczeniu usług drogą elektroniczną." width="" action={()=>{dispatch(checkTerms())}}/>  
            <br/>
