      ...
      <section className="mx-[50px] mb-[70px] mt-[35px]">
        ...
        <section className="flex flex-wrap justify-center">
          {subcategories.map(category =>
            <div 
              className={
                "flex flex-wrap w-[358px] h-[235px] justify-center items-center text-[32px] text-center leading-[40px] " + colors[Math.floor(Math.random()*colors.length)] + 
                " hover:scale-125 hover:text-yellow hover:cursor-pointer"
              } 
              onClick={()=>{goToProductsPageWithCategory(category)}}
            >{category.name}</div>
          )}
        </section>
